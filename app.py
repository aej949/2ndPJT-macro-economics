...
with st.expander("📈 대시보드 활용 및 그래프 해석 가이드 (클릭하여 확인)", expanded=False):
    st.markdown("""
    ### 1. 전제 조건 및 데이터 처리
    - **지수화 (Base=100):** 패널 A의 시계열 차트는 데이터의 단위가 다르므로, 조회 시작일의 가치를 100으로 설정하여 상대적 상승/하락폭을 비교합니다.
    - **변동률(Return) 기반 분석:** 상관관계(패널 B, C, D)는 단순 가격이 아닌 '수익률(변동률)'을 기준으로 계산합니다. 이는 가격의 절대적 높낮이보다는 **'움직임의 동행성'**에 집중하기 위함입니다.
    - **시차(Time-Lag):** 거시 경제 지표의 변화가 주식 시장에 반영되기까지의 시간을 고려하여, 지표 데이터를 0~6개월 밀어서 상관관계를 계산할 수 있습니다.

    ### 2. 패널별 그래프 해석 방법
    - **📌 패널 A (트렌드):** 거시 지표(금리)와 원자재 가격의 흐름을 봅니다. 금리는 우측 Y축(%), 원자재는 좌측 Y축(지수 100)을 기준으로 읽습니다.
    - **📌 패널 B (히트맵):** 거시 변수와 ETF 간의 상관계수를 보여줍니다. 
        - **+1 (붉은색):** 정방향으로 함께 움직임 (예: 유가 상승 시 에너지 ETF 상승)
        - **-1 (푸른색):** 역방향으로 반대로 움직임 (예: 금리 상승 시 부동산 ETF 하락)
    - **📌 패널 C (상세 분석):** 선택한 섹터와 가장 관련 깊은 지표 2개를 겹쳐서 봅니다. 
        - **회색 영역(Spread):** ETF 변동률과 1순위 지표 변동률 사이의 격차를 의미합니다. 격차가 벌어질수록 두 지표 간의 일시적인 괴리가 발생했음을 뜻합니다.
    - **📌 패널 D (산점도):** 방향성과 민감도를 직관적으로 확인합니다.
        - **점의 분포:** 점들이 추세선(빨간선)에 촘촘히 모여있을수록 상관관계가 강합니다.
        - **추세선 기울기:** 기울기가 가파를수록 거시 지표 변화에 ETF가 민감하게 반응함을 의미합니다.
    """)
