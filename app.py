...
st.caption("💡 **패널 A 해석:** 금리는 우측 Y축(%), 원자재는 좌측 Y축(조회시작일=100)을 기준으로 합니다. 거시 경제의 절대적 흐름(인플레이션과 조달 비용)을 확인하세요.")
st.markdown("---")
...
if not df_transformed[etf_cols].empty:
    ...
    st.markdown("""
    > **💡 패널 B 해석 (히트맵)**
    > - **붉은색 (+1):** 두 지표가 같은 방향으로 움직입니다 (예: 유가 상승 시 에너지 ETF 수익률 상승).
    > - **푸른색 (-1):** 두 지표가 반대 방향으로 움직입니다 (예: 유가 상승 시 항공 ETF 수익률 하락).
    > - **0에 가까움:** 두 지표 간에 뚜렷한 선형적 관계가 관찰되지 않습니다.
    """)
else:
...
if not df_transformed.empty:
    ...
    st.markdown("""
    > **💡 패널 C 해석 (변동률 & 격차)**
    > - **검은 굵은 선:** 선택한 ETF의 수익률(변동폭)입니다.
    > - **회색 영역(Spread):** ETF 수익률과 1순위 상관지표 간의 **'격차'**입니다. 색칠된 면적이 넓을수록 두 자산 간의 움직임이 일시적으로 어긋났음을 뜻합니다.
    """)
    ...
    if len(scatter_df) > 1:
        ...
        st.markdown("""
        > **💡 패널 D 해석 (산점도 & 추세선)**
        > - **점의 분포:** 점들이 대각성 선(추세선) 주위에 밀집할수록 높은 상관관계를 의미하며 분석의 신뢰도가 높습니다.
        > - **기울기:** 추세선의 기울기가 가파를수록 해당 거시 지표가 소폭 변할 때 ETF 수익률이 민감하게(레버리지 효과 등) 반응함을 증명합니다.
        """)
